...
export function LoadCodePanel(): JSX.Element {
  ...
  const handleLoad = () => {
    ...
    try {
      ...
      addToast("Opcodes loaded successfully", "success");
    } catch (err) {
      const message = err instanceof Error ? err.message : "Failed to load opcodes";
  ...
  return (
    <Panel title="Load Code">
      <textarea
        ...
        rows={5}
        className="w-full resize-y rounded bg-[var(--panel-2)] px-2 py-1.5 font-mono text-[12px] text-[var(--text)] outline-none focus:ring-1 focus:ring-[var(--accent)]"
      />
      <div className="mt-2">
        <Button data-testid="loadcode-button" onClick={handleLoad}>
          Load Opcodes
        </Button>
      </div>
      <div className="mt-4">
