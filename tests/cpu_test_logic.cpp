...
TEST_F(CPULogicalTest, and_immediate_basic) {
  ...
  bus.cpu_write(0xFFFD, 0xAA);  // Load 0xAA (10101010) into A
  execute_cycles(2);

  // Execute AND immediate
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::AND_IMM);
  bus.cpu_write(0xFFFF, 0x55);  // AND with 0x55 (01010101)
  execute_cycles(2);
...
TEST_F(CPULogicalTest, and_all_bits_set) {
  ...
  bus.cpu_write(0xFFFC, (nes::u8)nes::Opcode::LDA_IMM);
  bus.cpu_write(0xFFFD, 0xFF);  // Load 0xFF (11111111) into A
  execute_cycles(2);

  // Execute AND immediate with all bits set
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::AND_IMM);
  bus.cpu_write(0xFFFF, 0xFF);  // AND with 0xFF (11111111)
  execute_cycles(2);
...
TEST_F(CPULogicalTest, and_no_bits_set) {
  ...
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::AND_IMM);
  bus.cpu_write(0xFFFF, 0x00);  // AND with 0x00 (00000000)
  execute_cycles(2);
...
TEST_F(CPULogicalTest, and_negative_flag) {
  ...
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::AND_IMM);
  bus.cpu_write(0xFFFF, 0x80);  // AND with 0x80 (10000000)
  execute_cycles(2);
...
TEST_F(CPULogicalTest, and_combination) {
  ...
  bus.cpu_write(0xFFFC, (nes::u8)nes::Opcode::LDA_IMM);
  bus.cpu_write(0xFFFD, 0xF0);  // Load 0xF0 (11110000) into A
  execute_cycles(2);

  // Execute AND immediate with different bit pattern
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::AND_IMM);
  bus.cpu_write(0xFFFF, 0x0F);  // AND with 0x0F (00001111)
  execute_cycles(2);
...
TEST_F(CPULogicalTest, and_specific_pattern) {
  ...
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::AND_IMM);
  bus.cpu_write(0xFFFF, 0xAA);  // AND with 0xAA (10101010)
  execute_cycles(2);
...
TEST_F(CPULogicalTest, and_mask_operation) {
  ...
  bus.cpu_write(0xFFFC, (nes::u8)nes::Opcode::LDA_IMM);
  bus.cpu_write(0xFFFD, 0x5A);  // Load 0x5A (01011010) into A
  execute_cycles(2);

  // Execute AND immediate as a mask to keep only lower 4 bits
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::AND_IMM);
  bus.cpu_write(0xFFFF, 0x0F);  // AND with 0x0F (00001111) - lower 4 bits mask
  execute_cycles(2);
...
TEST_F(CPULogicalTest, and_zero_page) {
  ...
  bus.cpu_write(0xFFFC, (nes::u8)nes::Opcode::LDA_IMM);
  bus.cpu_write(0xFFFD, 0xFF);  // Load 0xFF into A
  execute_cycles(2);

  // Set up value in zero page
  bus.cpu_write(0x42, 0x55);  // Put 0x55 at zero page address 0x42

  // Execute AND zero page
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::AND_ZPG);
...
TEST_F(CPULogicalTest, and_absolute) {
  // Load accumulator with value
  bus.cpu_write(0xFFFC, (nes::u8)nes::Opcode::LDA_IMM);
  bus.cpu_write(0xFFFD, 0xF0);  // Load 0xF0 into A
  execute_cycles(2);

  // Set up value in memory
  bus.cpu_write(0x1234, 0x0F);  // Put 0x0F at address 0x1234

  // Execute AND absolute
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::AND_ABS);
...
TEST_F(CPULogicalTest, and_absolute_x) {
  ...
  bus.cpu_write(0xFFFF, 0xAA);  // Load 0xAA into A
  execute_cycles(2);

  // Set up value in memory
  bus.cpu_write(0x1244, 0x55);  // 0x1234 + 0x10 = 0x1244, put 0x55 there

  // Execute AND absolute X-indexed
  bus.cpu_write(0x0000, (nes::u8)nes::Opcode::AND_ABX);
...
TEST_F(CPULogicalTest, and_absolute_x_page_crossing) {
  ...
  // Load accumulator with value
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::LDA_IMM);
  bus.cpu_write(0xFFFF, 0xF0);  // Load 0xF0 into A
  execute_cycles(2);

  // Set up value in memory
  bus.cpu_write(0x12FE, 0x0F);  // 0x1200 + 0xFF = 0x12FF (page crossing), put 0x0F there
...
TEST_F(CPULogicalTest, and_sequence) {
  // Load accumulator with initial value
  bus.cpu_write(0xFFFC, (nes::u8)nes::Opcode::LDA_IMM);
  bus.cpu_write(0xFFFD, 0xFF);  // Load 0xFF (all bits set) into A
  execute_cycles(2);

  // First AND: 0xFF AND 0xF0 = 0xF0
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::AND_IMM);
  bus.cpu_write(0xFFFF, 0xF0);  // AND with 0xF0 (high nibble mask)
  execute_cycles(2);
  ...
  bus.cpu_write(0x0000, (nes::u8)nes::Opcode::AND_IMM);
  bus.cpu_write(0x0001, 0x33);  // AND with 0x33 (00110011)
  execute_cycles(2);
  ...
  bus.cpu_write(0x0002, (nes::u8)nes::Opcode::AND_IMM);
  bus.cpu_write(0x0003, 0x0F);  // AND with 0x0F (low nibble mask)
  execute_cycles(2);
  EXPECT_EQ(cpu.get_accumulator(), 0x00);
  EXPECT_TRUE(cpu.get_flag(nes::Flag::ZERO));
  EXPECT_FALSE(cpu.get_flag(nes::Flag::NEGATIVE));
...
TEST_F(CPULogicalTest, and_flags_not_affected) {
  // Set some flags
  bus.cpu_write(0xFFFC, (nes::u8)nes::Opcode::SEC_IMP);  // Set carry flag
  execute_cycles(2);

  // Load accumulator with value
  bus.cpu_write(0xFFFD, (nes::u8)nes::Opcode::LDA_IMM);
  bus.cpu_write(0xFFFE, 0xFF);
  execute_cycles(2);
  ...
  // Execute AND
  bus.cpu_write(0xFFFF, (nes::u8)nes::Opcode::AND_IMM);
...
TEST_F(CPULogicalTest, bit_zero_page_basic) {
  // Load accumulator with initial value
  bus.cpu_write(0xFFFC, (nes::u8)nes::Opcode::LDA_IMM);
  bus.cpu_write(0xFFFD, 0x0F);  // Load 0x0F (00001111) into A
  execute_cycles(2);

  // Set up test value in zero page
  bus.cpu_write(0x42, 0xF0);  // Put 0xF0 (11110000) at zero page address 0x42

  // Execute BIT zero page
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::BIT_ZPG);
  bus.cpu_write(0xFFFF, 0x42);  // Zero page address
  execute_cycles(3);
...
TEST_F(CPULogicalTest, bit_flags_clear) {
  ...
  // Set up test value in zero page
  bus.cpu_write(0x42, 0x0F);  // Put 0x0F (00001111) at zero page address 0x42
...
TEST_F(CPULogicalTest, bit_non_zero_and_high_bits_set) {
  ...
  // Set up test value in zero page
  bus.cpu_write(0x42, 0xC3);  // Put 0xC3 (11000011) at zero page address 0x42
...
TEST_F(CPULogicalTest, bit_zero_and_bit7_set_bit6_clear) {
  ...
  // Set up test value in zero page
  bus.cpu_write(0x42, 0x80);  // Put 0x80 (10000000) at zero page address 0x42
...
TEST_F(CPULogicalTest, bit_zero_and_bit7_clear_bit6_set) {
  ...
  // Set up test value in zero page
  bus.cpu_write(0x42, 0x40);  // Put 0x40 (01000000) at zero page address 0x42
...
TEST_F(CPULogicalTest, bit_absolute) {
  ...
  bus.cpu_write(0x1234, 0xCC);  // Put 0xCC (11001100) at address 0x1234

  // Execute BIT absolute
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::BIT_ABS);
  bus.cpu_write(0xFFFF, 0x34);  // Low byte of address
  bus.cpu_write(0x0000, 0x12);  // High byte of address
  execute_cycles(4);
...
TEST_F(CPULogicalTest, bit_other_flags_not_affected) {
  ...
  // Execute BIT zero page
  bus.cpu_write(0xFFFF, (nes::u8)nes::Opcode::BIT_ZPG);
...
TEST_F(CPULogicalTest, bit_testing_specific_bits) {
  ...
  bus.cpu_write(0xFFFC, (nes::u8)nes::Opcode::LDA_IMM);
  bus.cpu_write(0xFFFD, 0x08);  // Load 0x08 (00001000) into A
  execute_cycles(2);

  // Case 1: Bit 3 is set in memory
  bus.cpu_write(0x42, 0x08);  // Put 0x08 (00001000) at zero page address 0x42
  ...
  bus.cpu_write(0x43, 0x04);  // Put 0x04 (00000100) at zero page address 0x43

  // Execute BIT zero page
  bus.cpu_write(0x0000, (nes::u8)nes::Opcode::BIT_ZPG);
  bus.cpu_write(0x0001, 0x43);  // Zero page address
  execute_cycles(3);
...
TEST_F(CPULogicalTest, bit_copy_status_bits) {
  ...
  bus.cpu_write(0xFFFC, (nes::u8)nes::Opcode::LDA_IMM);
  bus.cpu_write(0xFFFD, 0xFF);  // Value doesn't matter as long as it won't result in Z=1
  execute_cycles(2);

  // First case: set both flags (bits 6 and 7 set)
  bus.cpu_write(0x42, 0xC0);  // Put 0xC0 (11000000) at zero page address 0x42
  ...
  bus.cpu_write(0x43, 0x80);  // Put 0x80 (10000000) at zero page address 0x43
  ...
  bus.cpu_write(0x44, 0x40);  // Put 0x40 (01000000) at zero page address 0x44

  // Execute BIT zero page
  bus.cpu_write(0x0002, (nes::u8)nes::Opcode::BIT_ZPG);
  bus.cpu_write(0x0003, 0x44);  // Zero page address
  execute_cycles(3);
...
TEST_F(CPULogicalTest, eor_immediate_basic) {
  ...
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::EOR_IMM);
  bus.cpu_write(0xFFFF, 0x55);  // EOR with 0x55 (01010101)
  execute_cycles(2);
...
TEST_F(CPULogicalTest, eor_same_value) {
  ...
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::EOR_IMM);
  bus.cpu_write(0xFFFF, 0xAA);  // EOR with 0xAA (10101010)
  execute_cycles(2);
...
TEST_F(CPULogicalTest, eor_with_zero) {
  ...
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::EOR_IMM);
  bus.cpu_write(0xFFFF, 0x00);  // EOR with 0x00 (00000000)
  execute_cycles(2);
...
TEST_F(CPULogicalTest, eor_negative_to_positive) {
  ...
  bus.cpu_write(0xFFFC, (nes::u8)nes::Opcode::LDA_IMM);
  bus.cpu_write(0xFFFD, 0x80);  // Load 0x80 (10000000) into A
  execute_cycles(2);

  // Execute EOR immediate with value that flips bit 7
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::EOR_IMM);
  bus.cpu_write(0xFFFF, 0x80);  // EOR with 0x80 (10000000)
  execute_cycles(2);
...
TEST_F(CPULogicalTest, eor_positive_to_negative) {
  ...
  bus.cpu_write(0xFFFC, (nes::u8)nes::Opcode::LDA_IMM);
  bus.cpu_write(0xFFFD, 0x00);  // Load 0x00 (00000000) into A
  execute_cycles(2);
...
TEST_F(CPULogicalTest, eor_bit_toggle) {
  ...
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::EOR_IMM);
  bus.cpu_write(0xFFFF, 0x0F);  // EOR with 0x0F (00001111) to toggle lower 4 bits
  execute_cycles(2);
...
TEST_F(CPULogicalTest, eor_zero_page) {
  ...
  // Set up value in zero page
  bus.cpu_write(0x42, 0x0F);  // Put 0x0F at zero page address 0x42

  // Execute EOR zero page
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::EOR_ZPG);
  bus.cpu_write(0xFFFF, 0x42);  // Zero page address
  execute_cycles(3);
...
TEST_F(CPULogicalTest, eor_absolute) {
  // Load accumulator with value
  bus.cpu_write(0xFFFC, (nes::u8)nes::Opcode::LDA_IMM);
  bus.cpu_write(0xFFFD, 0xAA);  // Load 0xAA into A
  execute_cycles(2);

  // Set up value in memory
  bus.cpu_write(0x1234, 0xAA);  // Put 0xAA at address 0x1234

  // Execute EOR absolute
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::EOR_ABS);
  bus.cpu_write(0xFFFF, 0x34);  // Low byte of address
  bus.cpu_write(0x0000, 0x12);  // High byte of address
  execute_cycles(4);
...
TEST_F(CPULogicalTest, eor_absolute_x) {
  ...
  // Set up value in memory
  bus.cpu_write(0x1244, 0x0F);  // 0x1234 + 0x10 = 0x1244, put 0x0F there

  // Execute EOR absolute X-indexed
  bus.cpu_write(0x0000, (nes::u8)nes::Opcode::EOR_ABX);
  bus.cpu_write(0x0001, 0x34);  // Low byte of base address
  bus.cpu_write(0x0002, 0x12);  // High byte of base address
  execute_cycles(4);            // No page crossing
...
TEST_F(CPULogicalTest, eor_absolute_x_page_crossing) {
  ...
  // Load accumulator with value
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::LDA_IMM);
  bus.cpu_write(0xFFFF, 0x55);  // Load 0x55 into A
  execute_cycles(2);

  // Set up value in memory
  bus.cpu_write(0x12FE, 0xAA);  // 0x1200 + 0xFF = 0x12FF (page crossing), put 0xAA there
...
TEST_F(CPULogicalTest, eor_sequence) {
  ...
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::EOR_IMM);
  bus.cpu_write(0xFFFF, 0xFF);  // EOR with 0xFF (all bits toggled)
  execute_cycles(2);
  EXPECT_EQ(cpu.get_accumulator(), 0xFF);
  EXPECT_TRUE(cpu.get_flag(nes::Flag::NEGATIVE));

  // Second EOR: 0xFF EOR 0xAA = 0x55
  bus.cpu_write(0x0000, (nes::u8)nes::Opcode::EOR_IMM);
  bus.cpu_write(0x0001, 0xAA);  // EOR with 0xAA (10101010)
  execute_cycles(2);
  EXPECT_EQ(cpu.get_accumulator(), 0x55);
  EXPECT_FALSE(cpu.get_flag(nes::Flag::NEGATIVE));

  // Third EOR: 0x55 EOR 0xFF = 0xAA
  bus.cpu_write(0x0002, (nes::u8)nes::Opcode::EOR_IMM);
  bus.cpu_write(0x0003, 0xFF);  // EOR with 0xFF (all bits toggled again)
  execute_cycles(2);
  EXPECT_EQ(cpu.get_accumulator(), 0xAA);
  EXPECT_TRUE(cpu.get_flag(nes::Flag::NEGATIVE));
}
...
TEST_F(CPULogicalTest, eor_flags_not_affected) {
  ...
  // Execute EOR
  bus.cpu_write(0xFFFF, (nes::u8)nes::Opcode::EOR_IMM);
...
TEST_F(CPULogicalTest, eor_toggling_bits) {
  // Load accumulator with initial value
  bus.cpu_write(0xFFFC, (nes::u8)nes::Opcode::LDA_IMM);
  bus.cpu_write(0xFFFD, 0x34);  // Load 0x34 (00110100) into A
  execute_cycles(2);

  // Toggle bit 5 using EOR
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::EOR_IMM);
  bus.cpu_write(0xFFFF, 0x20);  // EOR with 0x20 (00100000)
  execute_cycles(2);
  ...
  bus.cpu_write(0x0000, (nes::u8)nes::Opcode::EOR_IMM);
  bus.cpu_write(0x0001, 0x90);  // EOR with 0x90 (10010000)
  execute_cycles(2);
...
TEST_F(CPULogicalTest, ora_immediate_basic) {
  ...
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::ORA_IMM);
  bus.cpu_write(0xFFFF, 0x55);  // OR with 0x55 (01010101)
  execute_cycles(2);
...
TEST_F(CPULogicalTest, ora_with_zero) {
  ...
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::ORA_IMM);
  bus.cpu_write(0xFFFF, 0x00);  // OR with 0x00 (00000000)
  execute_cycles(2);
...
TEST_F(CPULogicalTest, ora_negative_flag) {
  ...
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::ORA_IMM);
  bus.cpu_write(0xFFFF, 0x80);  // OR with 0x80 (10000000)
  execute_cycles(2);
...
TEST_F(CPULogicalTest, ora_combination) {
  ...
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::ORA_IMM);
  bus.cpu_write(0xFFFF, 0x0F);  // OR with 0x0F (00001111)
  execute_cycles(2);
...
TEST_F(CPULogicalTest, ora_zero_page) {
  ...
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::ORA_ZPG);
  bus.cpu_write(0xFFFF, 0x42);  // Zero page address
  execute_cycles(3);
...
TEST_F(CPULogicalTest, ora_absolute) {
  // Load accumulator with value
  bus.cpu_write(0xFFFC, (nes::u8)nes::Opcode::LDA_IMM);
  bus.cpu_write(0xFFFD, 0x0A);  // Load 0x0A into A
  execute_cycles(2);

  // Set up value in memory
  bus.cpu_write(0x1234, 0x50);  // Put 0x50 at address 0x1234

  // Execute ORA absolute
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::ORA_ABS);
  bus.cpu_write(0xFFFF, 0x34);  // Low byte of address
  bus.cpu_write(0x0000, 0x12);  // High byte of address
  execute_cycles(4);
...
TEST_F(CPULogicalTest, ora_absolute_x) {
  ...
  // Load accumulator with value
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::LDA_IMM);
  bus.cpu_write(0xFFFF, 0x0A);  // Load 0x0A into A
  execute_cycles(2);

  // Set up value in memory
  bus.cpu_write(0x1244, 0x50);  // 0x1234 + 0x10 = 0x1244, put 0x50 there

  // Execute ORA absolute X-indexed
  bus.cpu_write(0x0000, (nes::u8)nes::Opcode::ORA_ABX);
  bus.cpu_write(0x0001, 0x34);  // Low byte of base address
  bus.cpu_write(0x0002, 0x12);  // High byte of base address
  execute_cycles(4);            // No page crossing
...
TEST_F(CPULogicalTest, ora_absolute_x_page_crossing) {
  ...
  // Set up value in memory
  bus.cpu_write(0x12FE, 0x80);  // 0x1200 + 0xFF = 0x12FF (page crossing), put 0x80 there
...
TEST_F(CPULogicalTest, ora_sequence) {
  ...
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::ORA_IMM);
  bus.cpu_write(0xFFFF, 0x01);  // OR with 0x01 (bit 0 set)
  execute_cycles(2);
  ...
  bus.cpu_write(0x0000, (nes::u8)nes::Opcode::ORA_IMM);
  bus.cpu_write(0x0001, 0x02);  // OR with 0x02 (bit 1 set)
  execute_cycles(2);
  ...
  bus.cpu_write(0x0002, (nes::u8)nes::Opcode::ORA_IMM);
  bus.cpu_write(0x0003, 0x04);  // OR with 0x04 (bit 2 set)
  execute_cycles(2);
  ...
  bus.cpu_write(0x0004, (nes::u8)nes::Opcode::ORA_IMM);
  bus.cpu_write(0x0005, 0x80);  // OR with 0x80 (bit 7 set)
  execute_cycles(2);
...
TEST_F(CPULogicalTest, ora_flags_not_affected) {
  ...
  // Execute ORA
  bus.cpu_write(0xFFFF, (nes::u8)nes::Opcode::ORA_IMM);
  bus.cpu_write(0x0000, 0x55);
  execute_cycles(2);
...
TEST_F(CPULogicalTest, ora_setting_bits) {
  ...
  // Set bit 0 using ORA
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::ORA_IMM);
  bus.cpu_write(0xFFFF, 0x01);  // OR with 0x01 (00000001)
  execute_cycles(2);
  ...
  bus.cpu_write(0x0000, (nes::u8)nes::Opcode::ORA_IMM);
  bus.cpu_write(0x0001, 0xC0);  // OR with 0xC0 (11000000)
  execute_cycles(2);
