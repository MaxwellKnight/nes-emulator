...
TEST_F(CPUIncrementDecrement, INX_Basic) {
  ...
  bus.cpu_write(0xFFFD, 0x41);
  execute_cycles(2);

  // Increment X
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::INX_IMP);
...
TEST_F(CPUIncrementDecrement, INY_Basic) {
  ...
  // Increment Y
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::INY_IMP);
...
TEST_F(CPUIncrementDecrement, INC_ZPG_Basic) {
  ...
  bus.cpu_write(0xFFFC, (nes::u8)nes::Opcode::INC_ZPG);
  bus.cpu_write(0xFFFD, 0x10);  // Zero page address
  bus.cpu_write(0x0010, 0x42);  // Initial value at zero page address
  execute_cycles(5);            // INC_ZPG takes 5 cycles

  // Verify the value was incremented
  EXPECT_EQ(bus.cpu_read(0x0010), 0x43);
  EXPECT_FALSE(cpu.get_flag(nes::Flag::ZERO));
  EXPECT_FALSE(cpu.get_flag(nes::Flag::NEGATIVE));
}
...
TEST_F(CPUIncrementDecrement, INC_ZPG_Zero) {
  ...
  bus.cpu_write(0xFFFC, (nes::u8)nes::Opcode::INC_ZPG);
  bus.cpu_write(0xFFFD, 0x10);  // Zero page address
  bus.cpu_write(0x0010, 0xFF);  // Initial value that will wrap to 0
  execute_cycles(5);            // INC_ZPG takes 5 cycles

  // Verify the value wrapped to zero
  EXPECT_EQ(bus.cpu_read(0x0010), 0x00);
  EXPECT_TRUE(cpu.get_flag(nes::Flag::ZERO));
  EXPECT_FALSE(cpu.get_flag(nes::Flag::NEGATIVE));
}
...
TEST_F(CPUIncrementDecrement, INC_ZPG_Negative) {
  ...
  bus.cpu_write(0xFFFC, (nes::u8)nes::Opcode::INC_ZPG);
  bus.cpu_write(0xFFFD, 0x10);  // Zero page address
  bus.cpu_write(0x0010, 0x7F);  // Value that will become negative when incremented
  execute_cycles(5);            // INC_ZPG takes 5 cycles

  // Verify the value and negative flag
  EXPECT_EQ(bus.cpu_read(0x0010), 0x80);
  EXPECT_FALSE(cpu.get_flag(nes::Flag::ZERO));
  EXPECT_TRUE(cpu.get_flag(nes::Flag::NEGATIVE));
}
...
TEST_F(CPUIncrementDecrement, INC_ZPX_Basic) {
  ...
  bus.cpu_write(0xFFFC, (nes::u8)nes::Opcode::LDX_IMM);
  bus.cpu_write(0xFFFD, 0x05);
  execute_cycles(2);

  // Increment value using zero page X-indexed addressing
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::INC_ZPX);
  bus.cpu_write(0xFFFF, 0x10);  // Base zero page address
  bus.cpu_write(0x0015, 0x42);  // Value at 0x10 + X (0x05)
  execute_cycles(6);            // INC_ZPX takes 6 cycles

  // Verify the value was incremented
  EXPECT_EQ(bus.cpu_read(0x0015), 0x43);
  EXPECT_FALSE(cpu.get_flag(nes::Flag::ZERO));
  EXPECT_FALSE(cpu.get_flag(nes::Flag::NEGATIVE));
}
...
TEST_F(CPUIncrementDecrement, INC_ABS_Basic) {
  ...
  bus.cpu_write(0xFFFC, (nes::u8)nes::Opcode::INC_ABS);
  bus.cpu_write(0xFFFD, 0x34);  // Low byte of address
  bus.cpu_write(0xFFFE, 0x12);  // High byte of address
  bus.cpu_write(0x1234, 0x42);  // Value at absolute address
  execute_cycles(6);            // INC_ABS takes 6 cycles

  // Verify the value was incremented
  EXPECT_EQ(bus.cpu_read(0x1234), 0x43);
  EXPECT_FALSE(cpu.get_flag(nes::Flag::ZERO));
  EXPECT_FALSE(cpu.get_flag(nes::Flag::NEGATIVE));
}
...
TEST_F(CPUIncrementDecrement, INC_ABX_Basic) {
  ...
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::INC_ABX);
  bus.cpu_write(0xFFFF, 0x30);  // Low byte of base address
  bus.cpu_write(0x0000, 0x12);  // High byte of base address
  bus.cpu_write(0x1235, 0x42);  // Value at 0x1230 + X (0x05)
  execute_cycles(7);            // INC_ABX takes 7 cycles

  // Verify the value was incremented
  EXPECT_EQ(bus.cpu_read(0x1235), 0x43);
  EXPECT_FALSE(cpu.get_flag(nes::Flag::ZERO));
  EXPECT_FALSE(cpu.get_flag(nes::Flag::NEGATIVE));
}
...
TEST_F(CPUIncrementDecrement, DEC_ZPG_Basic) {
  // Set up a zero page address with an initial value
  bus.cpu_write(0xFFFC, (nes::u8)nes::Opcode::DEC_ZPG);
  bus.cpu_write(0xFFFD, 0x10);  // Zero page address
  bus.cpu_write(0x0010, 0x42);  // Initial value at zero page address
  execute_cycles(5);            // DEC_ZPG takes 5 cycles

  // Verify the value was decremented
  EXPECT_EQ(bus.cpu_read(0x0010), 0x41);
  EXPECT_FALSE(cpu.get_flag(nes::Flag::ZERO));
  EXPECT_FALSE(cpu.get_flag(nes::Flag::NEGATIVE));
}
...
TEST_F(CPUIncrementDecrement, DEC_ZPG_Zero) {
  ...
  bus.cpu_write(0xFFFC, (nes::u8)nes::Opcode::DEC_ZPG);
  bus.cpu_write(0xFFFD, 0x10);  // Zero page address
  bus.cpu_write(0x0010, 0x01);  // Initial value that will become 0
  execute_cycles(5);            // DEC_ZPG takes 5 cycles
...
TEST_F(CPUIncrementDecrement, DEC_ZPG_Negative) {
  ...
  bus.cpu_write(0xFFFC, (nes::u8)nes::Opcode::DEC_ZPG);
  bus.cpu_write(0xFFFD, 0x10);  // Zero page address
  bus.cpu_write(0x0010, 0x00);  // Initial value that will become 0xFF (negative)
  execute_cycles(5);            // DEC_ZPG takes 5 cycles

  // Verify the value became 0xFF
  EXPECT_EQ(bus.cpu_read(0x0010), 0xFF);
  EXPECT_FALSE(cpu.get_flag(nes::Flag::ZERO));
  EXPECT_TRUE(cpu.get_flag(nes::Flag::NEGATIVE));
}
...
TEST_F(CPUIncrementDecrement, DEC_ZPX_Basic) {
  ...
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::DEC_ZPX);
  bus.cpu_write(0xFFFF, 0x10);  // Base zero page address
  bus.cpu_write(0x0015, 0x42);  // Value at 0x10 + X (0x05)
  execute_cycles(6);            // DEC_ZPX takes 6 cycles

  // Verify the value was decremented
  EXPECT_EQ(bus.cpu_read(0x0015), 0x41);
  EXPECT_FALSE(cpu.get_flag(nes::Flag::ZERO));
  EXPECT_FALSE(cpu.get_flag(nes::Flag::NEGATIVE));
}
...
TEST_F(CPUIncrementDecrement, DEC_ABS_Basic) {
  ...
  bus.cpu_write(0xFFFC, (nes::u8)nes::Opcode::DEC_ABS);
  bus.cpu_write(0xFFFD, 0x34);  // Low byte of address
  bus.cpu_write(0xFFFE, 0x12);  // High byte of address
  bus.cpu_write(0x1234, 0x42);  // Value at absolute address
  ...
  // Verify the value was decremented
  EXPECT_EQ(bus.cpu_read(0x1234), 0x41);
  EXPECT_FALSE(cpu.get_flag(nes::Flag::ZERO));
  EXPECT_FALSE(cpu.get_flag(nes::Flag::NEGATIVE));
}
...
TEST_F(CPUIncrementDecrement, DEC_ABX_Basic) {
  ...
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::DEC_ABX);
  bus.cpu_write(0xFFFF, 0x30);  // Low byte of base address
  bus.cpu_write(0x0000, 0x12);  // High byte of base address
  bus.cpu_write(0x1235, 0x42);  // Value at 0x1230 + X (0x05)
  ...
  // Verify the value was decremented
  EXPECT_EQ(bus.cpu_read(0x1235), 0x41);
  EXPECT_FALSE(cpu.get_flag(nes::Flag::ZERO));
  EXPECT_FALSE(cpu.get_flag(nes::Flag::NEGATIVE));
}
...
TEST_F(CPUIncrementDecrement, DEX_Basic) {
  ...
  // Decrement X
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::DEX_IMP);
...
TEST_F(CPUIncrementDecrement, DEY_Basic) {
  ...
  // Decrement Y
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::DEY_IMP);
