...
TEST_F(CPUStackTest, pha_basic) {
  ...
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::PHA_IMP);
  ...
  EXPECT_EQ(bus.cpu_read(0x01FF), 0x42);
...
TEST_F(CPUStackTest, pha_multiple_pushes) {
  ...
  // Push second value
  bus.cpu_write(0xFFFF, (nes::u8)nes::Opcode::LDA_IMM);
  bus.cpu_write(0x0000, 0x37);
  execute_cycles(2);

  bus.cpu_write(0x0001, (nes::u8)nes::Opcode::PHA_IMP);
  execute_cycles(3);

  // Verify stack state
  EXPECT_EQ(cpu.get_sp(), 0xFD);
  EXPECT_EQ(bus.cpu_read(0x01FF), 0x42);
  EXPECT_EQ(bus.cpu_read(0x01FE), 0x37);
}
...
TEST_F(CPUStackTest, php_basic) {
  // Set some flags
  bus.cpu_write(0xFFFC, (nes::u8)nes::Opcode::LDA_IMM);
  bus.cpu_write(0xFFFD, 0x00);  // This will set the zero flag
  execute_cycles(2);

  // Push processor status
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::PHP_IMP);
  ...
  EXPECT_EQ(bus.cpu_read(0x01FF), expected_status);
}

TEST_F(CPUStackTest, php_all_flags) {
  // Set various flags using operations
  bus.cpu_write(0xFFFC, (nes::u8)nes::Opcode::LDA_IMM);
  bus.cpu_write(0xFFFD, 0x80);  // Sets negative flag
  execute_cycles(2);
...
TEST_F(CPUStackTest, pla_basic) {
  ...
  // Clear accumulator
  bus.cpu_write(0xFFFF, (nes::u8)nes::Opcode::LDA_IMM);
  bus.cpu_write(0x0000, 0x00);
  execute_cycles(2);

  // Pull value from stack
  bus.cpu_write(0x0001, (nes::u8)nes::Opcode::PLA_IMP);
...
TEST_F(CPUStackTest, pla_negative_flag) {
  ...
  // Pull value from stack
  bus.cpu_write(0xFFFF, (nes::u8)nes::Opcode::PLA_IMP);
  execute_cycles(4);
...
TEST_F(CPUStackTest, plp_basic) {
  ...
  bus.cpu_write(0xFFFC, (nes::u8)nes::Opcode::PHP_IMP);
  execute_cycles(3);

  // Modify some flags
  bus.cpu_write(0xFFFD, (nes::u8)nes::Opcode::LDA_IMM);
  bus.cpu_write(0xFFFE, 0x00);  // Sets zero flag
  execute_cycles(2);

  // Pull processor status
  bus.cpu_write(0xFFFF, (nes::u8)nes::Opcode::PLP_IMP);
  ...
  nes::u8 expected_status = (bus.cpu_read(0x01FF) & ~0x10) | (cpu.get_status() & 0x10);
...
TEST_F(CPUStackTest, plp_preserves_break_flag) {
  ...
  // Pull processor status
  bus.cpu_write(0x0001, (nes::u8)nes::Opcode::PLP_IMP);
  execute_cycles(4);
