...
TEST_F(CPUStackTest, pha_basic) {
  ...
  bus.write(0xFFFE, (nes::u8)nes::Opcode::PHA);
  execute_cycles(3); // PHA takes 3 cycles
...
TEST_F(CPUStackTest, pha_multiple_pushes) {
  ...
  bus.write(0x0001, (nes::u8)nes::Opcode::PHA);
  execute_cycles(3);
...
TEST_F(CPUStackTest, php_basic) {
  ...
  bus.write(0xFFFE, (nes::u8)nes::Opcode::PHP);
  execute_cycles(3); // PHP takes 3 cycles
...
TEST_F(CPUStackTest, php_all_flags) {
  ...
  nes::u8 expected_status =
      cpu.get_status() | 0x30; // With BREAK and UNUSED set
  EXPECT_EQ(bus.read(0x01FF), expected_status);
  EXPECT_EQ(cpu.get_sp(), 0xFE);
}
...
TEST_F(CPUStackTest, pla_basic) {
  ...
  bus.write(0x0001, (nes::u8)nes::Opcode::PLA);
  execute_cycles(4); // PLA takes 4 cycles
...
TEST_F(CPUStackTest, pla_negative_flag) {
  ...
  // Pull value from stack
  bus.write(0xFFFF, (nes::u8)nes::Opcode::PLA);
  execute_cycles(4);
...
TEST_F(CPUStackTest, plp_basic) {
  ...
  bus.write(0xFFFC, (nes::u8)nes::Opcode::PHP);
  execute_cycles(3);
  ...
  bus.write(0xFFFF, (nes::u8)nes::Opcode::PLP);
  execute_cycles(4); // PLP takes 4 cycles

  // Original status had BREAK and UNUSED set when pushed,
  // but BREAK flag should be unchanged when pulled
  nes::u8 expected_status =
      (bus.read(0x01FF) & ~0x10) | (cpu.get_status() & 0x10);
...
TEST_F(CPUStackTest, plp_preserves_break_flag) {
  ...
  // Pull processor status
  bus.write(0x0001, (nes::u8)nes::Opcode::PLP);
  execute_cycles(4);
