...
TEST_F(CPUShiftRotateTest, asl_accumulator) {
  ...
  bus.cpu_write(0xFFFD, 0x45);  // binary: 01000101
  execute_cycles(2);

  // Execute ASL on accumulator
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::ASL_ACC);
...
TEST_F(CPUShiftRotateTest, asl_accumulator_carry) {
  // Load value into accumulator
  bus.cpu_write(0xFFFC, (nes::u8)nes::Opcode::LDA_IMM);
  bus.cpu_write(0xFFFD, 0x80);  // binary: 10000000
  execute_cycles(2);
...
TEST_F(CPUShiftRotateTest, asl_absolute) {
  ...
  bus.cpu_write(0x0042, 0x45);  // binary: 01000101

  // Execute ASL on absolute address
  bus.cpu_write(0xFFFC, (nes::u8)nes::Opcode::ASL_ABS);
  bus.cpu_write(0xFFFD, 0x42);
  bus.cpu_write(0xFFFE, 0x00);  // Address 0x0042
  execute_cycles(6);            // ASL Absolute takes 6 cycles

  EXPECT_EQ(cpu.get_remaining_cycles(), 0);
  EXPECT_EQ(bus.cpu_read(0x0042), 0x8A);  // binary: 10001010
  EXPECT_FALSE(cpu.get_flag(nes::Flag::CARRY));
  EXPECT_TRUE(cpu.get_flag(nes::Flag::NEGATIVE));
  EXPECT_FALSE(cpu.get_flag(nes::Flag::ZERO));
}
...
TEST_F(CPUShiftRotateTest, asl_absolute_carry_and_zero) {
  ...
  bus.cpu_write(0x0042, 0x80);  // binary: 10000000
  ...
  EXPECT_EQ(cpu.get_remaining_cycles(), 0);
  EXPECT_EQ(bus.cpu_read(0x0042), 0x00);  // binary: 00000000
  EXPECT_TRUE(cpu.get_flag(nes::Flag::CARRY));
  EXPECT_FALSE(cpu.get_flag(nes::Flag::NEGATIVE));
  EXPECT_TRUE(cpu.get_flag(nes::Flag::ZERO));
}
...
TEST_F(CPUShiftRotateTest, asl_absolute_x) {
  ...
  // Set up test value in memory (using lower address range)
  bus.cpu_write(0x0052, 0x45);  // binary: 01000101 (at address 0x0042 + 0x10)

  // Execute ASL on absolute X-indexed address
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::ASL_ABX);
  bus.cpu_write(0xFFFF, 0x42);
  bus.cpu_write(0x0000, 0x00);  // Base address 0x0042, X = 0x10, final = 0x0052
  execute_cycles(7);            // ASL Absolute X takes 7 cycles

  EXPECT_EQ(cpu.get_remaining_cycles(), 0);
  EXPECT_EQ(bus.cpu_read(0x0052), 0x8A);  // binary: 10001010
  EXPECT_FALSE(cpu.get_flag(nes::Flag::CARRY));
  EXPECT_TRUE(cpu.get_flag(nes::Flag::NEGATIVE));
  EXPECT_FALSE(cpu.get_flag(nes::Flag::ZERO));
}
...
TEST_F(CPUShiftRotateTest, asl_absolute_x_carry_and_zero) {
  ...
  // Set up test value in memory (using lower address range)
  bus.cpu_write(0x0052, 0x80);  // binary: 10000000 (at address 0x0042 + 0x10)
  ...
  EXPECT_EQ(cpu.get_remaining_cycles(), 0);
  EXPECT_EQ(bus.cpu_read(0x0052), 0x00);  // binary: 00000000
  EXPECT_TRUE(cpu.get_flag(nes::Flag::CARRY));
  EXPECT_FALSE(cpu.get_flag(nes::Flag::NEGATIVE));
  EXPECT_TRUE(cpu.get_flag(nes::Flag::ZERO));
}
...
TEST_F(CPUShiftRotateTest, asl_zero_page) {
  ...
  bus.cpu_write(0x42, 0x45);  // binary: 01000101

  // Execute ASL on zero page address
  bus.cpu_write(0xFFFC, (nes::u8)nes::Opcode::ASL_ZPG);
  bus.cpu_write(0xFFFD, 0x42);  // Zero page address 0x42
  execute_cycles(5);            // ASL Zero Page takes 5 cycles

  EXPECT_EQ(cpu.get_remaining_cycles(), 0);
  EXPECT_EQ(bus.cpu_read(0x42), 0x8A);  // binary: 10001010
  EXPECT_FALSE(cpu.get_flag(nes::Flag::CARRY));
  EXPECT_TRUE(cpu.get_flag(nes::Flag::NEGATIVE));
  EXPECT_FALSE(cpu.get_flag(nes::Flag::ZERO));
}
...
TEST_F(CPUShiftRotateTest, asl_zero_page_carry_and_zero) {
  // Set up test value in zero page
  bus.cpu_write(0x42, 0x80);  // binary: 10000000
  ...
  EXPECT_EQ(bus.cpu_read(0x42), 0x00);  // binary: 00000000
  EXPECT_TRUE(cpu.get_flag(nes::Flag::CARRY));
  EXPECT_FALSE(cpu.get_flag(nes::Flag::NEGATIVE));
  EXPECT_TRUE(cpu.get_flag(nes::Flag::ZERO));
}
...
TEST_F(CPUShiftRotateTest, asl_zero_page_x) {
  ...
  // Set up test value in zero page
  bus.cpu_write(0x52, 0x45);  // binary: 01000101 (at zero page address 0x42 + 0x10)

  // Execute ASL on zero page X-indexed address
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::ASL_ZPX);
  bus.cpu_write(0xFFFF,
                0x42);  // Base zero page address 0x42, X = 0x10, final = 0x52
  execute_cycles(6);    // ASL Zero Page X takes 6 cycles

  EXPECT_EQ(cpu.get_remaining_cycles(), 0);
  EXPECT_EQ(bus.cpu_read(0x52), 0x8A);  // binary: 10001010
  EXPECT_FALSE(cpu.get_flag(nes::Flag::CARRY));
  EXPECT_TRUE(cpu.get_flag(nes::Flag::NEGATIVE));
  EXPECT_FALSE(cpu.get_flag(nes::Flag::ZERO));
}
...
TEST_F(CPUShiftRotateTest, asl_zero_page_x_wrapping) {
  ...
  // Set up test value in zero page
  bus.cpu_write(0x41, 0x45);  // binary: 01000101 (at wrapped address (0x42 + 0xFF) &
                              // 0xFF = 0x41)
  ...
  EXPECT_EQ(cpu.get_remaining_cycles(), 0);
  EXPECT_EQ(bus.cpu_read(0x41), 0x8A);  // binary: 10001010
  EXPECT_FALSE(cpu.get_flag(nes::Flag::CARRY));
  EXPECT_TRUE(cpu.get_flag(nes::Flag::NEGATIVE));
  EXPECT_FALSE(cpu.get_flag(nes::Flag::ZERO));
}
...
TEST_F(CPUShiftRotateTest, lsr_accumulator) {
  // Load value into accumulator
  bus.cpu_write(0xFFFC, (nes::u8)nes::Opcode::LDA_IMM);
  bus.cpu_write(0xFFFD, 0x8A);  // binary: 10001010
  execute_cycles(2);

  // Execute LSR on accumulator
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::LSR_ACC);
...
TEST_F(CPUShiftRotateTest, lsr_accumulator_carry) {
  // Load value into accumulator
  bus.cpu_write(0xFFFC, (nes::u8)nes::Opcode::LDA_IMM);
  bus.cpu_write(0xFFFD, 0x01);  // binary: 00000001
  execute_cycles(2);
...
TEST_F(CPUShiftRotateTest, lsr_absolute) {
  // Set up test value in memory (using lower address range 0x0000-0x1FFF)
  bus.cpu_write(0x0042, 0x8A);  // binary: 10001010

  // Execute LSR on absolute address
  bus.cpu_write(0xFFFC, (nes::u8)nes::Opcode::LSR_ABS);
  bus.cpu_write(0xFFFD, 0x42);
  bus.cpu_write(0xFFFE, 0x00);  // Address 0x0042
  execute_cycles(6);            // LSR Absolute takes 6 cycles

  EXPECT_EQ(cpu.get_remaining_cycles(), 0);
  EXPECT_EQ(bus.cpu_read(0x0042), 0x45);  // binary: 01000101
  EXPECT_FALSE(cpu.get_flag(nes::Flag::CARRY));
  EXPECT_FALSE(cpu.get_flag(nes::Flag::NEGATIVE));
  EXPECT_FALSE(cpu.get_flag(nes::Flag::ZERO));
}
...
TEST_F(CPUShiftRotateTest, lsr_absolute_carry_and_zero) {
  // Set up test value in memory (using lower address range)
  bus.cpu_write(0x0042, 0x01);  // binary: 00000001
...
TEST_F(CPUShiftRotateTest, lsr_absolute_x) {
  ...
  // Set up test value in memory (using lower address range)
  bus.cpu_write(0x0052, 0x8A);  // binary: 10001010 (at address 0x0042 + 0x10)

  // Execute LSR on absolute X-indexed address
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::LSR_ABX);
  bus.cpu_write(0xFFFF, 0x42);
  bus.cpu_write(0x0000, 0x00);  // Base address 0x0042, X = 0x10, final = 0x0052
  execute_cycles(7);            // LSR Absolute X takes 7 cycles

  EXPECT_EQ(cpu.get_remaining_cycles(), 0);
  EXPECT_EQ(bus.cpu_read(0x0052), 0x45);  // binary: 01000101
  EXPECT_FALSE(cpu.get_flag(nes::Flag::CARRY));
  EXPECT_FALSE(cpu.get_flag(nes::Flag::NEGATIVE));
  EXPECT_FALSE(cpu.get_flag(nes::Flag::ZERO));
}
...
TEST_F(CPUShiftRotateTest, lsr_absolute_x_carry_and_zero) {
  ...
  // Set up test value in memory (using lower address range)
  bus.cpu_write(0x0052, 0x01);  // binary: 00000001 (at address 0x0042 + 0x10)
...
TEST_F(CPUShiftRotateTest, lsr_zero_page) {
  // Set up test value in zero page
  bus.cpu_write(0x42, 0x8A);  // binary: 10001010

  // Execute LSR on zero page address
  bus.cpu_write(0xFFFC, (nes::u8)nes::Opcode::LSR_ZPG);
  bus.cpu_write(0xFFFD, 0x42);  // Zero page address 0x42
  execute_cycles(5);            // LSR Zero Page takes 5 cycles

  EXPECT_EQ(cpu.get_remaining_cycles(), 0);
  EXPECT_EQ(bus.cpu_read(0x42), 0x45);  // binary: 01000101
  EXPECT_FALSE(cpu.get_flag(nes::Flag::CARRY));
  EXPECT_FALSE(cpu.get_flag(nes::Flag::NEGATIVE));
  EXPECT_FALSE(cpu.get_flag(nes::Flag::ZERO));
}
...
TEST_F(CPUShiftRotateTest, lsr_zero_page_carry_and_zero) {
  // Set up test value in zero page
  bus.cpu_write(0x42, 0x01);  // binary: 00000001
...
TEST_F(CPUShiftRotateTest, lsr_zero_page_x) {
  ...
  // Set up test value in zero page
  bus.cpu_write(0x52, 0x8A);  // binary: 10001010 (at zero page address 0x42 + 0x10)

  // Execute LSR on zero page X-indexed address
  bus.cpu_write(0xFFFE, (nes::u8)nes::Opcode::LSR_ZPX);
  bus.cpu_write(0xFFFF,
                0x42);  // Base zero page address 0x42, X = 0x10, final = 0x52
  execute_cycles(6);    // LSR Zero Page X takes 6 cycles

  EXPECT_EQ(cpu.get_remaining_cycles(), 0);
  EXPECT_EQ(bus.cpu_read(0x52), 0x45);  // binary: 01000101
  EXPECT_FALSE(cpu.get_flag(nes::Flag::CARRY));
  EXPECT_FALSE(cpu.get_flag(nes::Flag::NEGATIVE));
  EXPECT_FALSE(cpu.get_flag(nes::Flag::ZERO));
}
...
TEST_F(CPUShiftRotateTest, lsr_zero_page_x_wrapping) {
  ...
  // Set up test value in zero page
  bus.cpu_write(0x41, 0x8A);  // binary: 10001010 (at wrapped address (0x42 + 0xFF) &
                              // 0xFF = 0x41)
  ...
  EXPECT_EQ(cpu.get_remaining_cycles(), 0);
  EXPECT_EQ(bus.cpu_read(0x41), 0x45);  // binary: 01000101
  EXPECT_FALSE(cpu.get_flag(nes::Flag::CARRY));
  EXPECT_FALSE(cpu.get_flag(nes::Flag::NEGATIVE));
  EXPECT_FALSE(cpu.get_flag(nes::Flag::ZERO));
}
...
TEST_F(CPUShiftRotateTest, rol_accumulator_carry_clear) {
  ...
  bus.cpu_write(0xFFFC, (nes::u8)nes::Opcode::CLC_IMP);
  execute_cycles(2);

  // Load value into accumulator
  bus.cpu_write(0xFFFD, (nes::u8)nes::Opcode::LDA_IMM);
  bus.cpu_write(0xFFFE, 0x45);  // binary: 01000101
  execute_cycles(2);

  // Execute ROL on accumulator
  bus.cpu_write(0xFFFF, (nes::u8)nes::Opcode::ROL_ACC);
...
TEST_F(CPUShiftRotateTest, rol_accumulator_carry_set) {
  ...
  bus.cpu_write(0xFFFC, (nes::u8)nes::Opcode::SEC_IMP);
  execute_cycles(2);
...
TEST_F(CPUShiftRotateTest, rol_accumulator_carry_out) {
  ...
  // Load value into accumulator
  bus.cpu_write(0xFFFD, (nes::u8)nes::Opcode::LDA_IMM);
  bus.cpu_write(0xFFFE, 0x80);  // binary: 10000000
  execute_cycles(2);
...
TEST_F(CPUShiftRotateTest, rol_absolute_carry_clear) {
  ...
  bus.cpu_write(0xFFFD, (nes::u8)nes::Opcode::ROL_ABS);
  bus.cpu_write(0xFFFE, 0x42);
  bus.cpu_write(0xFFFF, 0x00);  // Address 0x0042
...
TEST_F(CPUShiftRotateTest, rol_absolute_carry_set) {
  ...
  EXPECT_EQ(bus.cpu_read(0x0042), 0x8B);  // binary: 10001011
  EXPECT_FALSE(cpu.get_flag(nes::Flag::CARRY));
  EXPECT_TRUE(cpu.get_flag(nes::Flag::NEGATIVE));
  EXPECT_FALSE(cpu.get_flag(nes::Flag::ZERO));
}
...
TEST_F(CPUShiftRotateTest, rol_absolute_x) {
  ...
  // Set X register
  bus.cpu_write(0xFFFD, (nes::u8)nes::Opcode::LDX_IMM);
  bus.cpu_write(0xFFFE, 0x10);  // X = 0x10
  execute_cycles(2);
  ...
  bus.cpu_write(0xFFFF, (nes::u8)nes::Opcode::ROL_ABX);
  bus.cpu_write(0x0000, 0x42);
  bus.cpu_write(0x0001, 0x00);  // Base address 0x0042, X = 0x10, final = 0x0052
...
TEST_F(CPUShiftRotateTest, rol_absolute_x_carry_in_and_out) {
  ...
  EXPECT_EQ(bus.cpu_read(0x0052), 0x01);  // binary: 00000001
  EXPECT_TRUE(cpu.get_flag(nes::Flag::CARRY));
  EXPECT_FALSE(cpu.get_flag(nes::Flag::NEGATIVE));
  EXPECT_FALSE(cpu.get_flag(nes::Flag::ZERO));
}
...
TEST_F(CPUShiftRotateTest, rol_zero_page) {
  ...
  bus.cpu_write(0xFFFD, (nes::u8)nes::Opcode::ROL_ZPG);
  bus.cpu_write(0xFFFE, 0x42);  // Zero page address 0x42
...
TEST_F(CPUShiftRotateTest, rol_zero_page_to_zero) {
  ...
  // Set up test value in zero page
  bus.cpu_write(0x42, 0x00);  // binary: 00000000
...
TEST_F(CPUShiftRotateTest, rol_zero_page_x) {
  ...
  bus.cpu_write(0xFFFF, (nes::u8)nes::Opcode::ROL_ZPX);
  bus.cpu_write(0x0000, 0x42);  // Base zero page address 0x42, X = 0x10, final = 0x52
  execute_cycles(6);            // ROL Zero Page X takes 6 cycles

  EXPECT_EQ(cpu.get_remaining_cycles(), 0);
  EXPECT_EQ(bus.cpu_read(0x52), 0x8B);  // binary: 10001011
  EXPECT_FALSE(cpu.get_flag(nes::Flag::CARRY));
  EXPECT_TRUE(cpu.get_flag(nes::Flag::NEGATIVE));
  EXPECT_FALSE(cpu.get_flag(nes::Flag::ZERO));
}
...
TEST_F(CPUShiftRotateTest, rol_zero_page_x_wrapping) {
  ...
  // Set X register to cause wrap-around
  bus.cpu_write(0xFFFD, (nes::u8)nes::Opcode::LDX_IMM);
  bus.cpu_write(0xFFFE, 0xFF);  // X = 0xFF
  execute_cycles(2);

  // Set up test value in zero page
  bus.cpu_write(0x41, 0x80);  // binary: 10000000 (at wrapped address (0x42 + 0xFF) & 0xFF = 0x41)
  ...
  EXPECT_EQ(cpu.get_remaining_cycles(), 0);
  EXPECT_EQ(bus.cpu_read(0x41), 0x00);  // binary: 00000000
  EXPECT_TRUE(cpu.get_flag(nes::Flag::CARRY));
  EXPECT_FALSE(cpu.get_flag(nes::Flag::NEGATIVE));
  EXPECT_TRUE(cpu.get_flag(nes::Flag::ZERO));
}
...
TEST_F(CPUShiftRotateTest, rol_multiple_operations) {
  ...
  bus.cpu_write(0xFFFD, (nes::u8)nes::Opcode::LDA_IMM);
  bus.cpu_write(0xFFFE, 0x01);  // Load A with 0000 0001
  execute_cycles(2);
  ...
  bus.cpu_write(0x0006, (nes::u8)nes::Opcode::ROL_ACC);
  execute_cycles(2);
  ...
  bus.cpu_write(0x0007, (nes::u8)nes::Opcode::ROL_ACC);
  execute_cycles(2);
...
TEST_F(CPUShiftRotateTest, ror_accumulator_carry_clear) {
  ...
  // Load value into accumulator
  bus.cpu_write(0xFFFD, (nes::u8)nes::Opcode::LDA_IMM);
  bus.cpu_write(0xFFFE, 0x8A);  // binary: 10001010
  execute_cycles(2);

  // Execute ROR on accumulator
  bus.cpu_write(0xFFFF, (nes::u8)nes::Opcode::ROR_ACC);
...
TEST_F(CPUShiftRotateTest, ror_accumulator_carry_out) {
  ...
  // Load value into accumulator
  bus.cpu_write(0xFFFD, (nes::u8)nes::Opcode::LDA_IMM);
  bus.cpu_write(0xFFFE, 0x01);  // binary: 00000001
  execute_cycles(2);
...
TEST_F(CPUShiftRotateTest, ror_absolute_carry_clear) {
  ...
  bus.cpu_write(0xFFFD, (nes::u8)nes::Opcode::ROR_ABS);
  bus.cpu_write(0xFFFE, 0x42);
  bus.cpu_write(0xFFFF, 0x00);  // Address 0x0042
...
TEST_F(CPUShiftRotateTest, ror_absolute_carry_set) {
  ...
  EXPECT_EQ(bus.cpu_read(0x0042), 0xC5);  // binary: 11000101
  EXPECT_FALSE(cpu.get_flag(nes::Flag::CARRY));
  EXPECT_TRUE(cpu.get_flag(nes::Flag::NEGATIVE));
  EXPECT_FALSE(cpu.get_flag(nes::Flag::ZERO));
}
...
TEST_F(CPUShiftRotateTest, ror_absolute_x) {
  ...
  bus.cpu_write(0xFFFF, (nes::u8)nes::Opcode::ROR_ABX);
  bus.cpu_write(0x0000, 0x42);
  bus.cpu_write(0x0001, 0x00);  // Base address 0x0042, X = 0x10, final = 0x0052
...
TEST_F(CPUShiftRotateTest, ror_absolute_x_carry_in_and_out) {
  ...
  EXPECT_EQ(bus.cpu_read(0x0052), 0x80);  // binary: 10000000
  EXPECT_TRUE(cpu.get_flag(nes::Flag::CARRY));
  EXPECT_TRUE(cpu.get_flag(nes::Flag::NEGATIVE));
  EXPECT_FALSE(cpu.get_flag(nes::Flag::ZERO));
}
...
TEST_F(CPUShiftRotateTest, ror_zero_page) {
  ...
  bus.cpu_write(0xFFFD, (nes::u8)nes::Opcode::ROR_ZPG);
  bus.cpu_write(0xFFFE, 0x42);  // Zero page address 0x42
...
TEST_F(CPUShiftRotateTest, ror_zero_page_x) {
  ...
  bus.cpu_write(0xFFFF, (nes::u8)nes::Opcode::ROR_ZPX);
  bus.cpu_write(0x0000, 0x42);  // Base zero page address 0x42, X = 0x10, final = 0x52
  execute_cycles(6);            // ROR Zero Page X takes 6 cycles

  EXPECT_EQ(cpu.get_remaining_cycles(), 0);
  EXPECT_EQ(bus.cpu_read(0x52), 0xC5);  // binary: 11000101
  EXPECT_FALSE(cpu.get_flag(nes::Flag::CARRY));
  EXPECT_TRUE(cpu.get_flag(nes::Flag::NEGATIVE));
  EXPECT_FALSE(cpu.get_flag(nes::Flag::ZERO));
}
...
TEST_F(CPUShiftRotateTest, ror_zero_page_x_wrapping) {
  ...
  // Set up test value in zero page
  bus.cpu_write(0x41, 0x01);  // binary: 00000001 (at wrapped address (0x42 + 0xFF) & 0xFF = 0x41)
...
TEST_F(CPUShiftRotateTest, ror_multiple_operations) {
  ...
  bus.cpu_write(0xFFFD, (nes::u8)nes::Opcode::LDA_IMM);
  bus.cpu_write(0xFFFE, 0x80);  // Load A with 1000 0000
  execute_cycles(2);
  ...
  bus.cpu_write(0x0000, (nes::u8)nes::Opcode::ROR_ACC);
  execute_cycles(2);
  EXPECT_EQ(cpu.get_accumulator(), 0x20);
  EXPECT_FALSE(cpu.get_flag(nes::Flag::CARRY));

  // Third ROR: 0010 0000 -> 0001 0000, carry = 0
  bus.cpu_write(0x0001, (nes::u8)nes::Opcode::ROR_ACC);
  execute_cycles(2);
  EXPECT_EQ(cpu.get_accumulator(), 0x10);
  EXPECT_FALSE(cpu.get_flag(nes::Flag::CARRY));

  // Fourth ROR: 0001 0000 -> 0000 1000, carry = 0
  bus.cpu_write(0x0002, (nes::u8)nes::Opcode::ROR_ACC);
  execute_cycles(2);
  EXPECT_EQ(cpu.get_accumulator(), 0x08);
  EXPECT_FALSE(cpu.get_flag(nes::Flag::CARRY));
  ...
  bus.cpu_write(0x0003, (nes::u8)nes::Opcode::ROR_ACC);
  execute_cycles(2);
  EXPECT_EQ(cpu.get_accumulator(), 0x04);
  EXPECT_FALSE(cpu.get_flag(nes::Flag::CARRY));

  // Sixth ROR: 0000 0100 -> 0000 0010, carry = 0
  bus.cpu_write(0x0004, (nes::u8)nes::Opcode::ROR_ACC);
  execute_cycles(2);
  EXPECT_EQ(cpu.get_accumulator(), 0x02);
  EXPECT_FALSE(cpu.get_flag(nes::Flag::CARRY));

  // Seventh ROR: 0000 0010 -> 0000 0001, carry = 0
  bus.cpu_write(0x0005, (nes::u8)nes::Opcode::ROR_ACC);
  execute_cycles(2);
  EXPECT_EQ(cpu.get_accumulator(), 0x01);
  EXPECT_FALSE(cpu.get_flag(nes::Flag::CARRY));
  EXPECT_FALSE(cpu.get_flag(nes::Flag::NEGATIVE));
  ...
  bus.cpu_write(0x0006, (nes::u8)nes::Opcode::ROR_ACC);
  execute_cycles(2);
  EXPECT_EQ(cpu.get_accumulator(), 0x00);
  EXPECT_TRUE(cpu.get_flag(nes::Flag::CARRY));
  EXPECT_FALSE(cpu.get_flag(nes::Flag::NEGATIVE));
  ...
  bus.cpu_write(0x0007, (nes::u8)nes::Opcode::ROR_ACC);
  execute_cycles(2);
  EXPECT_EQ(cpu.get_accumulator(), 0x80);
  EXPECT_FALSE(cpu.get_flag(nes::Flag::CARRY));
  EXPECT_TRUE(cpu.get_flag(nes::Flag::NEGATIVE));
